// 모바일 통계 — 데스크톱 "한눈에 보기"와 동일한 내용 (StatsWing 과 같은 집계)
...
function Block({ icon, label, value, sub, tone = 'ink' }) {
  ...
  return (
    <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-line/60">
      ...
      <p className="text-[26px] font-bold leading-none tabular-nums" style={{ color }}>
        {value}
      </p>
      {sub && <p className="mt-1.5 text-[12px] text-sub">{sub}</p>}
    </div>
  )
}
  ...
  return (
    <>
      <header className="flex h-14 shrink-0 items-center px-4">
        <h1 className="text-[20px] font-extrabold tracking-tight text-ink">통계</h1>
      </header>

      <div className="flex-1 overflow-y-auto px-4 pb-6">
        <p className="mb-3 px-1 text-[13px] font-semibold tracking-wide text-sub">한눈에 보기</p>
